...
def test_next_sync_committee_merkle_proof(spec, state):
    ...
    yield (
        "proof",
        {
            "leaf": f"0x{state.next_sync_committee.hash_tree_root().hex()}",
            "leaf_index": spec.NEXT_SYNC_COMMITTEE_INDEX,
            "branch": [
                f'0x{root.hex()}' for root in next_sync_committee_branch
            ],
        },
    )
...
def test_finality_root_merkle_proof(spec, state):
    ...
    yield (
        "proof",
        {
            "leaf": f"0x{state.finalized_checkpoint.root.hex()}",
            "leaf_index": spec.FINALIZED_ROOT_INDEX,
            "branch": [f'0x{root.hex()}' for root in finality_branch],
        },
    )
