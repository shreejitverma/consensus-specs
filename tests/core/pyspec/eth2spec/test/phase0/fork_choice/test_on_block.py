...
def test_proposer_boost(spec, state):
    ...
    time = (
        store.genesis_time + block.slot * spec.config.SECONDS_PER_SLOT
    ...
    time = store.genesis_time + block.slot * spec.config.SECONDS_PER_SLOT + spec.config.SECONDS_PER_SLOT
    on_tick_and_append_step(spec, store, time, test_steps)
